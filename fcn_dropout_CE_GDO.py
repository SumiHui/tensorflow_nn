...
def mnistdataclassical_dropout_CE_GDO(mnistDataDir="MNIST_data",batchSize=200,learningRate=0.01,epochSize=50,dropout=0.7):
    # 载入数据
    ...
    with tf.Session() as sess:
        ...
        with open('tensorboard/run%s.txt'%time.strftime('%m%d'), 'a') as f:
            f.write('----------------dropout_CE GradientDescentOptimizer chart%s---------------\n'% nowtime)
            f.write('%s\n'%time.strftime('%x %X'))
            f.write("Batch: %d \n" % batchSize)
            f.write("Epoch: %d \n" % epochSize)
            f.write("Dropout: %f \n" % dropout)
